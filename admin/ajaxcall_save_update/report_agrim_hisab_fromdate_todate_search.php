<?php
	...
	if($fromdate == ''){
		echo $table_header;
		echo "<tr><td colspan='6' class='cenText'>Please set from date !</tr></td>";
	} else if($todate == ''){
		echo $table_header;
		echo "<tr><td colspan='6' class='cenText'>Please set to date !</tr></td>";
	} else {		
		$fromdateNew 	= implode('-', array_reverse(explode('-', $fromdate)));
		$todateNew 		= implode('-', array_reverse(explode('-', $todate)));
		
		echo $table_header;
		$query = "SELECT * FROM agrim_hisab WHERE agrim_date BETWEEN '$fromdateNew' AND '$todateNew' AND project_name_id = '$project_name_id'";

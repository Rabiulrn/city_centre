...
$total_debit = 0; // initialize
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $total_debit = $row['debit'];
        if (is_null($total_debit) || $total_debit === '') {
            $total_debit = 0;
        }
    }
}

// Ensure all variables are numeric before calculation
$total_debit = floatval($total_debit);
$total_credit = isset($total_credit) ? floatval($total_credit) : 0;
$motor_vara_and_unload = isset($motor_vara_and_unload) ? floatval($motor_vara_and_unload) : 0;
...
<table width="100%" class="summary">
    <tr>
      ...
      <!-- <td class="hastext">
      <input type="text" placeholder="divided by 12" id="twelve"></input>
			<td style="min-width: 85px" id="output_t"></td>
      </td> -->

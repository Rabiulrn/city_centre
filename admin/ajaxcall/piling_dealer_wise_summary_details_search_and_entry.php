...
$_SESSION['dealerIdInput'] = $dealerId;
// echo $dealerId;
// $project_name_id = $_SESSION['project_name_id'];
// $edit_data_permission   = $_SESSION['edit_data'];
// $delete_data_permission = $_SESSION['delete_data'];

$project_name_id         = isset($_SESSION['project_name_id']) ? $_SESSION['project_name_id'] : '';
$edit_data_permission    = isset($_SESSION['edit_data']) ? $_SESSION['edit_data'] : '';
$delete_data_permission  = isset($_SESSION['delete_data']) ? $_SESSION['delete_data'] : '';
    ...
    <div class="upper">
      ...
      ";
        ...
        <tr>
          ...
          <td><b><?php echo isset($mot_beg) ? $mot_beg : ''; ?></b></td>
      ...
      <table border="1" id="detailsEtryTable">
        ...
        <tr>
          ...
          <?php
            ...
            if ($customersId && $customersId->num_rows > 0) {
                $row = $customersId->fetch_assoc();
                $largestId = $row['sl'];
            } else {
                $largestId = 'SL-100000'; // Make sure the prefix is uppercase and consistent
            }

            // Extract number safely
            $matches = preg_replace('/\D/', '', $largestId); // Remove non-digits
            $matches = ($matches !== '') ? (int)$matches : 100000; // Default if empty
        ...
        <table id="detailsNewTable2">
          ...
          <tbody>
            ...
            while ($rows = $result->fetch_assoc()) {
              $challan_date = isset($rows['challan_date']) ? $rows['challan_date'] : '';
              $so_date      = isset($rows['so_date']) ? $rows['so_date'] : '';
              $dates        = isset($rows['dates']) ? $rows['dates'] : '';

              // Format dates
              $format_challan_date = ($challan_date && $challan_date != '0000-00-00') ? date("d-m-Y", strtotime($challan_date)) : '';
              $format_so_date      = ($so_date && $so_date != '0000-00-00') ? date("d-m-Y", strtotime($so_date)) : '';
              $format_dates        = ($dates && $dates != '0000-00-00') ? date("d-m-Y", strtotime($dates)) : '';
              echo "<tr>";
              echo "<td>" . $rows['sl'] . "</td>";
            ...
            //   echo "<td>" . $rows['address'] . "</td>";
              echo "<td>" . (isset($rows['date']) ? $rows['date'] : '') . "</td>";
              echo "<td>" . (isset($rows['marfot_name']) ? $rows['marfot_name'] : '') . "</td>";
              echo "<td>" . (isset($rows['description']) ? $rows['description'] : '') . "</td>";
              echo "<td>" . (isset($rows['cap_no']) ? $rows['cap_no'] : '') . "</td>";
              // echo "<td>" . $format_challan_date . "</td>";
              echo "<td>" . (isset($rows['particulars']) ? $rows['particulars'] : '') . "</td>";        
              // echo "<td>" . $format_dates . "</td>";
              echo "<td>" . (isset($rows['bill_cash']) ? $rows['bill_cash'] : '') . "</td>";
              echo "<td>" . (isset($rows['address']) ? $rows['address'] : '') . "</td>";
              echo "<td>" . (isset($rows['piling_count']) ? $rows['piling_count'] : '') . "</td>";
              echo "<td>" . (isset($rows['feet']) ? $rows['feet'] : '') . "</td>";
              // echo "<td>" . $rows['fee'] . "</td>";
              echo "<td>" . (isset($rows['paras']) ? $rows['paras'] : '') . "</td>";
              echo "<td>" . (isset($rows['piling_bill']) ? $rows['piling_bill'] : '') . "</td>";
              echo "<td>" . (isset($rows['money_back']) ? $rows['money_back'] : '') . "</td>";

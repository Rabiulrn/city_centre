    ...
    if($search_date == 'alldates'){
        ...
        if ($read) {
            while ($row = $read->fetch_assoc()) {
                ...
                $qry = "SELECT * FROM debit_group_data WHERE group_id = '$khotos_khat_id' AND group_description = '$biboron' AND project_name_id = '$project_name_id'";
                $reads = $db->select($qry);
        ...
        if ($read) {
            while ($row = $read->fetch_assoc()) {
                ...
                $group_wise_total_pay = 0;
                $group_wise_total_due = 0;
                $qry = "SELECT * FROM debit_group_data WHERE group_id = '$khotos_khat_id' AND entry_date = '$search_date' AND group_description = '$biboron' AND project_name_id = '$project_name_id'";
                $reads = $db->select($qry);
                if (mysqli_num_rows($reads) > 0) {

  ...
  <div style="display: flex;">
    ...
    <div class="lower">
      ...
      <td><?php echo $total_shift; ?></td> -->
          <td class="hastext"></td>
          <td><?php echo isset($company_paona) ? $company_paona : ''; ?></td>
        ...
        <tr>
          ...
          <td><b><?php echo isset($nij_paona) ? $nij_paona : ''; ?></b></td>
    ...
    <div class="scrolling-div" id="scrolling-entry-div">
      ...
      <table border="1" id="detailsEtryTable">
        <tr>
          ...
          <td class="widthPercent3" style="display: none;">et_paras</td>
        ...
        <tr>
          ...
          // $newId = 'SL-' . $newNumber;
          $matches = preg_replace('/\D/', '', $largestId);
          $newNumber = (int)$matches + 1;   // cast ensures numeric
          $newId = 'SL-' . $newNumber;
          ?>
...
<?php
$sql = "SELECT * FROM details_brick WHERE dealer_id='$dealerId' AND project_name_id = '$project_name_id'";
$result = $db->select($sql);
if ($result) {
        ...
        <table id="detailsNewTable2">
          ...
          <tbody>
            <?php
              ...
              // challan_date
              if (isset($rows['challan_date']) && $rows['challan_date'] != '0000-00-00') {
                  $format_challan_date = date("d-m-Y", strtotime($rows['challan_date']));
              } else {
                  $format_challan_date = '';
              }

              // so_date
              if (isset($rows['so_date']) && $rows['so_date'] != '0000-00-00') {
                  $format_so_date = date("d-m-Y", strtotime($rows['so_date']));
              } else {
                  $format_so_date = '';
              }

              // dates
              if (isset($rows['dates']) && $rows['dates'] != '0000-00-00') {
                  $format_dates = date("d-m-Y", strtotime($rows['dates']));
              } else {
                  $format_dates = '';
              }
              echo "<tr>";

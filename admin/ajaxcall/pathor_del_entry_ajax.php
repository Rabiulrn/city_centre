<?php 
  ...
  if(isset($rod_details_id)){
    ...
    $sql = "SELECT ton,partculars,particulars FROM details_pathor WHERE id = '$rod_details_id'";
    $show = $db->select($sql);
    if ($show) {
      ...
      $sql_update = "UPDATE stocks_pathor 
               SET `ton` = `ton` - '$del_ton' 
               WHERE partculars ='$del_part' 
                 AND particulars ='$del_parti' 
                 AND `ton` - '$del_ton' >= 0 
               ORDER BY ton DESC 
               LIMIT 1";
      ...
      $sql = "DELETE FROM details_pathor WHERE id = '$rod_details_id'";

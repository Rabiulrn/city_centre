<?php 
    session_start();
    if(!isset($_SESSION['username'])){
        header('location:../index.php');
    }
    $project_name_id = $_SESSION['project_name_id'];
    require '../config/config.php';
    require '../lib/database.php';
    $db = new Database();


    $motor_cash      = isset($_POST['motor_cash']) ? trim($_POST['motor_cash']) : '';
    $unload          = isset($_POST['unload']) ? trim($_POST['unload']) : '';
    $car_rent_redeem = isset($_POST['car_rent_redeem']) ? trim($_POST['car_rent_redeem']) : '';
    $Information     = isset($_POST['information']) ? trim($_POST['information']) : '';
    $buyer_id        = isset($_POST['buyer_id']) ? trim($_POST['buyer_id']) : '';
    // $delear_id      = trim($_POST['delear_id']);
    $delear_id       = isset($_SESSION['dealerIdInput']) ? trim($_SESSION['dealerIdInput']) : '';
    $address         = isset($_POST['address']) ? trim($_POST['address']) : '';
    $sl_no           = isset($_POST['sl_no']) ? trim($_POST['sl_no']) : '';
    $delivery_no     = isset($_POST['delivery_no']) ? trim($_POST['delivery_no']) : '';
    $motor           = isset($_POST['motor']) ? trim($_POST['motor']) : '';
    $motor_no        = isset($_POST['motor_no']) ? trim($_POST['motor_no']) : '';

    // $delivery_date  = trim($_POST['delivery_date']);
    if($_POST['delivery_date'] == ''){
      $delivery_date = $_POST['delivery_date'];
    } else {
      $postDateArr    = explode('-', $_POST['delivery_date']);
      $delivery_date  = $postDateArr['2'].'-'.$postDateArr['1'].'-'.$postDateArr['0'];
    }
    // $dates          = trim($_POST['dates']);
    if($_POST['dates'] == ''){
      $dates = $_POST['dates'];
    } else {
      $postDateArr2   = explode('-', $_POST['dates']);
      $dates          = $postDateArr2['2'].'-'.$postDateArr2['1'].'-'.$postDateArr2['0'];
    }
    $partculars    = isset($_POST['partculars']) ? trim($_POST['partculars']) : '';
    $particulars   = isset($_POST['particulars']) ? trim($_POST['particulars']) : '';
    $debit         = isset($_POST['debit']) ? trim($_POST['debit']) : '';
    $mm            = isset($_POST['mm']) ? trim($_POST['mm']) : '';
    $kg            = isset($_POST['kg']) ? trim($_POST['kg']) : '';
    $paras         = isset($_POST['paras']) ? trim($_POST['paras']) : '';
    $credit        = isset($_POST['credit']) ? trim($_POST['credit']) : '';
    $discount      = isset($_POST['discount']) ? trim($_POST['discount']) : '';
    $balance       = isset($_POST['balance']) ? trim($_POST['balance']) : '';
    $bundil        = isset($_POST['bundil']) ? trim($_POST['bundil']) : '';
    $total_paras   = isset($_POST['total_paras']) ? trim($_POST['total_paras']) : '';   

    $sql = "INSERT INTO details (motor_cash, unload, cars_rent_redeem, information, buyer_id, dealer_id, address, sl_no, delivery_no, motor, motor_no, delivery_date, dates, partculars, particulars, debit,   mm, kg, paras, credit, discount, balance, bundil, total_paras, project_name_id) VALUES('$motor_cash', '$unload', '$car_rent_redeem', '$Information', '$buyer_id', '$delear_id', '$address', '$sl_no', '$delivery_no', '$motor', '$motor_no', '$delivery_date', '$dates', '$partculars', '$particulars', '$debit', '$mm', '$kg', '$paras', '$credit ', '$discount', '$balance', '$bundil', '$total_paras', '$project_name_id')";

    $result = $db->insert($sql);
    if ($result) 
    {
        $sucMsg = "New Entry Saved Successfully.";
        $sucMsgPopup = "New Entry Saved Successfully.";
        echo $sucMsg;
    } else{
        echo "Error: " . $sql . "<br>" . $db->error;
    }
  
  
  ?>
<?php 
  ...
  $_SESSION['pageName'] = 'piling_hisab_entry';  
  $sucMsg = '';

  $sql = "SELECT category_id FROM piling_category ORDER BY id DESC LIMIT 1";
  ...
  if(isset($_POST['submit'])){
      ...
      if($submitBtn_value == 'Save'){
          $category_id     = $newId;
          $sql="INSERT INTO piling_category(category_id, category_name,project_name_id) VALUES ('$category_id','$category_name','$project_name_id')";
  ...
  if(isset($_GET['remove_id'])){
      ...
      $sql = "DELETE FROM piling_category WHERE id = '$delete_dealer'";
      if ($db->select($sql) === TRUE) {
...
<html>
<head>
    <title> পাইলিং ক্যাটাগরি এনট্রি</title>
    <meta charset="utf-8">
    ...
    <div class="bar_con">
        ...
        <div class="showDealerCon">
          <table >
            ...
            <?php
              $sql = "SELECT * FROM piling_category WHERE project_name_id = '$project_name_id' ";
              $show = $db->select($sql);
              if ($show) {
                  while ($rows = $show->fetch_assoc()){
                      echo "<tr>";

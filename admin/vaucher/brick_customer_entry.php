<?php 
	...
	$_SESSION['pageName'] = 'brick_customer_entry';	
	$sucMsg = '';


  	$sql = "SELECT customer_id FROM customers_brick ORDER BY id DESC LIMIT 1";
	...
	if(isset($_POST['submit'])){
		...
		if($submitBtn_value == 'Save'){
			...
			$sql="INSERT INTO customers_brick (customer_id, customer_name, address, mobile, buying_type, project_name_id) VALUES ('$customer_id', '$customer_name', '$address', '$mobile', '$buying_type', '$project_name_id')";
			...
			$sql="UPDATE customers_brick SET customer_name = '$customer_name', address = '$address', mobile = '$mobile', buying_type = '$buying_type' WHERE customer_id = '$customer_id'";
	...
	if(isset($_GET['remove_id'])){
		...
		$sql = "DELETE FROM customers_brick WHERE id = '$customer_delete'";
		if ($db->select($sql) === TRUE) {
	            ...
	            $('<div></div>').appendTo('body')
	                            ...
	                            .dialog({
	                                ...
	                                buttons: {
	                                    Yes: function () {
	                                        $(this).dialog("close");
	                                        $.get("brick_customer_entry.php?remove_id="+remove_id, function(data, status){
	                                          console.log(status);
	                                          if(status == 'success'){
	                                            window.location.href = 'brick_customer_entry.php';
	                                          }
	                                        });
	                                    },
	                                    No: function () {

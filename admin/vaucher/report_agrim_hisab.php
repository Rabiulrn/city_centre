...
<html lang="en">
<head>
	<title>অগ্রিম হিসাব রিপোর্ট</title>
	<meta charset="utf-8">
...
?> 
	...
	<div class="bar_con">
		...
		<div class="left_side_bar menu">
			<div id="left_all_menu_con">
				...
				<a href="../vaucher/report_agrim_hisab.php" class="active">অগ্রিম হিসাব</a>
			</div>	
		</div>
		<div class="main_bar">
			<?php
		  	...
		  	?>
		  	<div class="project_heading text-center" >      
		    	<h2 class="text-center" style="font-size: 23px; line-height: 22px;">অগ্রিম হিসাব</h2>
		    </div>
		  	...
		  	<div class="row">
				<div class="col-md-12">
					<div class="optionsCon">
						<span>
		    				<b>Date:</b>
		    				<select id="searchdate" style="width: 130px; height: 34px;">
		    					<option value="alldates">All Dates...</option>
		    					<?php
		    						$sql = "SELECT DISTINCT agrim_date FROM agrim_hisab WHERE project_name_id = '$project_name_id' ORDER BY agrim_date ASC";
		    						$rslt = $db->select($sql);
						            $row_no = mysqli_num_rows($rslt);
						            if ($rslt && $row_no > 0) {
						            	while ($row = $rslt->fetch_assoc()){
						            		$agrim_date_list = $row['agrim_date'];
						            		if($agrim_date_list == '0000-00-00'){
												$agrim_date_list ='';
												//then date will show 01/01/1970
						            		}
						            		echo "<option value='".$row['agrim_date']."'>". date("d-m-Y", strtotime($agrim_date_list)) ."</option>";
						            	}
						            }
		    					?>
		    				</select>
	...
	<script type="text/javascript">
		$(document).on('input', '#search', function(){
			function searchJoma(searchTxt){
			    $.ajax({
			        url: "../ajaxcall_save_update/report_agrim_hisab_search.php",
			        type: "post",
	...
	<script type="text/javascript">
		...
		function fromTodateSearch(fromdate, todate){
			$.ajax({
		        url: "../ajaxcall_save_update/report_agrim_hisab_fromdate_todate_search.php",
		        type: "post",
		        data: {
		        	fromdate 	: fromdate, 
		        	todate 		: todate,
		...
		function yearMonthSearch(monthvalue, yearvalue){
			$.ajax({
		        url: "../ajaxcall_save_update/report_agrim_hisab_year_month_search.php",
		        type: "post",
		        data: {
		...
		function dateSearch(searchdate){
			$.ajax({
		        url: "../ajaxcall_save_update/report_agrim_hisab_date_wise_search.php",
		        type: "post",
		        data: {
	...
	<script type="text/javascript">
		var all_row;
		function show_credit_name_wise_agrim(agrim_name) {
			all_row = $("#tableshow").html();
			$.ajax({
		        url: "../ajaxcall_save_update/report_agrim_hisab_name_wise_view.php",
		        type: "post",
		        data: {
		        	agrim_name 	: agrim_name,
		        },
		        success: function (response) {
		          // alert(response);
		          $('#tableshow').html(response);

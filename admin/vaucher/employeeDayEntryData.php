...
if ($data2 && $data2->num_rows > 0) {
    ...
    $imgName        = $display['employee_profile_pic'] ?? "";
    $beton          = $display['total_diner_beton'] ?? 0;
    $khaber_bill    = $display['total_khaber_bill'] ?? 0;
    $naster_bill    = $display['total_naster_bill'] ?? 0;
    $total_bonus    = $display['bonus'] ?? 0;
    $total_nea_joma = $display['nea_joma'] ?? 0;

    $naster_bill    = (float) $naster_bill;
$beton          = (float) $beton;
$total_bonus    = (float) $total_bonus;
$total_nea_joma = (float) $total_nea_joma;
...
$motPowna = $total_nea_joma - $beton;
    ...
    // for debugging - see why no data
    // echo "<p style='color:red'>No employee data found for this query.</p>";
}
            ...
            if ($imgName == "") { ?>
                ...
                <?php  } else { ?>
                    ...
                    <div class="raj_align">
                            <div style="margin-left: 10px;">মোবাইল নাম্বারঃ </div>
                            <div> 
                        <?php echo isset($display['employee_mobile_num']) ? $display['employee_mobile_num'] : 'N/A'; ?> 
                            </div>
                        </div>

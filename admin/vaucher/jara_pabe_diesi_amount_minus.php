...
if(isset($_POST['insertUpdateBtn'])) {
    ...
    if($insertUpdateBtn == 'Save Diesi Amount'){
        $query = "INSERT INTO entry_jara_pabe (jp_name,  jp_description, jp_amount, jp_date, jp_status, jara_pabe_id, project_name_id) VALUES ('$jp_name', '$jp_description', '$jp_amount', '$dates', 'sub','$data_id', '$project_name_id')";
      ...
      <div class="container">
        ...
        <?php            
            ...
            if(mysqli_num_rows($jara_pabe) == 1){
                ...
                $query = "SELECT SUM(jp_amount) as add_amount FROM entry_jara_pabe WHERE jara_pabe_id = '$data_id' AND jp_status ='add' AND project_name_id = '$project_name_id'";
                  $show = $db->select($query);
            ...
            $query = "SELECT * FROM entry_jara_pabe WHERE jara_pabe_id = '$data_id' AND jp_status ='sub' AND project_name_id = '$project_name_id' ORDER BY jp_date DESC";
            $show = $db->select($query);
              ...
              $('<div></div>').appendTo('body')
                              ...
                              .dialog({
                                  ...
                                  buttons: {
                                      Yes: function () {
                                          $(this).dialog("close");
                                          $.get("jara_pabe_diesi_amount_minus.php?data_id="+data_id+"&row_id="+row_id, function(data, status){
                                            // console.log(status);
                                            if(status == 'success'){
                                                window.location.href = "jara_pabe_diesi_amount_minus.php?data_id="+data_id;
                                            }
                                          });
                                      },
                                      No: function () {

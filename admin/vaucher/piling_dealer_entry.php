<?php 
  ...
  $_SESSION['pageName'] = 'piling_dealer_entry';  
  $sucMsg = '';

  $sql = "SELECT dealer_id FROM piling_dealer ORDER BY id DESC LIMIT 1";
  ...
  if(isset($_POST['submit'])){
      ...
      if($submitBtn_value == 'Save'){
          $dealer_id     = $newId;
          $sql="INSERT INTO piling_dealer (dealer_id, dealer_name, address, contact_person_name, mobile, project_name_id) VALUES ('$dealer_id','$dealer_name', '$address', '$contact_person_name', '$mobile',  '$project_name_id')";
          ...
          $sql="UPDATE piling_dealer SET dealer_name = '$dealer_name', address = '$address', contact_person_name = '$contact_person_name', mobile = '$mobile' WHERE dealer_id = '$dealer_id'";
  ...
  if(isset($_GET['remove_id'])){
      ...
      $sql = "DELETE FROM piling_dealer WHERE id = '$delete_dealer'";
      if ($db->select($sql) === TRUE) {
...
<html>
<head>
    <title>পাইলিং ডিলার এনট্রি</title>
    <meta charset="utf-8">
                ...
                $('<div></div>').appendTo('body')
                                ...
                                .dialog({
                                    ...
                                    buttons: {
                                        Yes: function () {
                                            $(this).dialog("close");
                                            $.get("piling_dealer_entry.php?remove_id="+remove_id, function(data, status){
                                              console.log(status);
                                              if(status == 'success'){
                                                window.location.href = 'piling_dealer_entry.php';
                                              }
                                            });
                                        },
                                        No: function () {

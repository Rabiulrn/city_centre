<?php 
	...
	$db = new Database();
	$_SESSION['pageName'] = 'brick_report_customer';
...
?> 
	...
	<div class="bar_con">
		...
		<div class="left_side_bar menu">
			<div id="left_all_menu_con">
				...
				<!-- <a href="../vaucher/rod_report_others_category.php">রড ও অন্যান্ন ক্যাটাগরি</a> -->
				 <a href="../vaucher/brick_report_dealer.php">ডিলার অনুযায়ী </a>
				 <a href="../vaucher/brick_report_customer.php" class="active">কাস্টমার অনুযায়ী </a> 
		...
		<div class="main_bar">
					...
					<div class="optionsCon">
						<span style="position: relative; top: -13px;">
		    				...
		    				<select class="selctpik2" id="searchCustomer">
		    					...
		    					<?php
		    						$sql = "SELECT DISTINCT customer_id, customer_name FROM customers_brick WHERE project_name_id = '$project_name_id' ORDER BY customer_name ASC";
		    						$rslt = $db->select($sql);
			...
			<th style='border: 1px solid #777 !important;'>#</th>
    			...
    			<?php
    				$sql = "SELECT * FROM customers_brick WHERE project_name_id = '$project_name_id'";
    				...
    				if($result && $row_number > 0){
    					...
    					while($row = $result->fetch_assoc()){
					        ...
					        //Start Gari vara
					        	$motor_vara = 0;
						        $sql2 = "SELECT SUM(motor_vara) as motor_vara FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
								...
								$total_motor_vara = 0;
						        $sql3 = "SELECT SUM(motor_vara) as motor_vara FROM details_sell_brick WHERE project_name_id = '$project_name_id'";
						    ...
						    //Start khalas/Unload
						        $unload = 0;
						        $sql2 = "SELECT SUM(unload) as unload FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						                $unload = $row2['unload'];
								...
								$total_unload = 0;
								$sql3 = "SELECT SUM(unload) as unload FROM details_sell_brick WHERE  project_name_id = '$project_name_id'";
								$result3 = $db->select($sql3);
								if($result3->num_rows > 0){
									while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_credit/mot_mul
						        $total_credit = 0;
						        $sql2 = "SELECT SUM(credit) as credit FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
								...
								$total1_credit = 0;
						        $sql3 = "SELECT SUM(credit) as credit FROM details_sell_brick WHERE project_name_id = '$project_name_id'";
						        $result3 = $db->select($sql3);
						        if($result3->num_rows > 0){
						            while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_debit/joma
						        $total_debit = 0;
						        $sql2 = "SELECT SUM(debit) as debit FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
								...
								$total1_debit = 0;
						        $sql3 = "SELECT SUM(debit) as debit FROM details_sell_brick WHERE project_name_id = '$project_name_id'";
						        $result3 = $db->select($sql3);
						        if($result3->num_rows > 0){
						            while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_Balance/mot_jer
						        $total_balance = 0;
						        $sql2 = "SELECT SUM(balance) as balance FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
								...
								$total1_balance = 0;
						        $sql3 = "SELECT SUM(balance) as balance FROM details_sell_brick WHERE  project_name_id = '$project_name_id'";
						        $result3 = $db->select($sql3);
						        if($result3->num_rows > 0){
						            while($row3 = $result3->fetch_assoc()){
						    ...
						    //Start GB Bank Ganti
						        $gb_bank_ganti = 0;
						        $sql2 = "SELECT SUM(debit) as debit, id FROM details_sell_brick WHERE particulars = 'BG' AND customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						...
						//Start Total para/mot_mul_khoros_shoho
						        $paras = 0;
						        $sql2 = "SELECT SUM(paras) as paras FROM details_sell_brick WHERE customer_id = '$customer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
	...
	<script type="text/javascript">	
		...
		function fromTodateSearch(fromdate, todate){
			$.ajax({
		        url: "../ajaxcall_rod_report/brick_report_customer_fromdate_todate_search.php",
		        type: "post",
		        data: {
		        	fromdate 	: fromdate, 
		        	todate 		: todate,
		...
		function yearMonthSearch(monthvalue, yearvalue){
			$.ajax({
		        url: "../ajaxcall_rod_report/brick_report_customer_year_month_search.php",
		        type: "post",
		        data: {
	...
	<script type="text/javascript">
		$(document).on('input', '#search', function(){
			function searchByCustomerName(searchTxt){
			    $.ajax({
			        url: "../ajaxcall_rod_report/brick_report_customer_search.php",
			        type: "post",
		...
		function customerWiseSearch(searchCustomer){
			$.ajax({
		        url: "../ajaxcall_rod_report/brick_report_customer_name_wise_search.php",
		        type: "post",
		        data: {
		...
		$(document).on('change', '#searchCustomer', function(){
			...
			if(searchCustomer == 'allcustomers'){
				window.location = '../vaucher/brick_report_customer.php';
			} else {

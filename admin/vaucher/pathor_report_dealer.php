<?php 
	...
	$db = new Database();
	$_SESSION['pageName'] = 'pathor_report_dealer';
...
?> 
					...
					<div class="optionsCon">
						<span style="position: relative; top: -13px;">
		    				...
		    				<select class="selctpik2" id="searchDealer">
		    					...
		    					<?php
		    						$sql = "SELECT DISTINCT dealer_id, dealer_name FROM pathor_dealer WHERE project_name_id = '$project_name_id' ORDER BY dealer_name ASC";
		    						$rslt = $db->select($sql);
    			...
    			<?php
    				...
    				if($result && $row_number > 0){
    					...
    					while($row = $result->fetch_assoc()){
    						...
    						// $sql2 = "SELECT SUM(kg) as kg FROM details_pathor WHERE particulars LIKE '%500W%' AND dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
					        ...
					        //     $rod500w = 0;
					        // }
					        // $sql2 = "SELECT SUM(kg) as kg FROM details_pathor WHERE particulars LIKE '%400W%' AND dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
					        // $result2 = $db->select($sql2);
					        // if($result2->num_rows > 0){
					        //     while($row2 = $result2->fetch_assoc()){
					        //         $rod400w = $row2['kg'];
					        ...
					        // 	$total_motor = 0;
						    //     $sql2 = "SELECT SUM(motor) as motor FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						    //     $result2 = $db->select($sql2);
						    //     if($result2->num_rows > 0){
						    //         while($row2 = $result2->fetch_assoc()){
						    ...
						    //Start khalas/Unload
						        $unload = 0;
						        $sql2 = "SELECT SUM(unload) as unload FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						                $unload = $row2['unload'];
						    ...
						    // Start total total_credit/mot_mul
						        $total_credit = 0;
						        $sql2 = "SELECT SUM(credit) as credit FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						    ...
						    // Start total total_debit/joma
						        $total_debit = 0;
						        $sql2 = "SELECT SUM(debit) as debit FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						    ...
						    // Start total total_Balance/mot_jer
						        $total_balance = 0;
						        $sql2 = "SELECT SUM(balance) as balance FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						    ...
						    //Start GB Bank Ganti
						        $gb_bank_ganti = 0;
						        $sql2 = "SELECT SUM(debit) as debit, id FROM details_pathor WHERE particulars = 'BG' AND dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
						...
						//Start Total para/mot_mul_khoros_shoho
						        $paras = 0;
						        $sql2 = "SELECT SUM(paras) as paras FROM details_pathor WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						        $result2 = $db->select($sql2);
						        if($result2->num_rows > 0){
						            while($row2 = $result2->fetch_assoc()){
	...
	<script type="text/javascript">	
		...
		function fromTodateSearch(fromdate, todate){
			$.ajax({
		        url: "../ajaxcall_rod_report/pathor_report_dealer_fromdate_todate_search.php",
		        type: "post",
		        data: {
		        	fromdate 	: fromdate, 
		        	todate 		: todate,
		...
		function yearMonthSearch(monthvalue, yearvalue){
			$.ajax({
		        url: "../ajaxcall_rod_report/pathor_report_dealer_year_month_search.php",
		        type: "post",
		        data: {
	...
	<script type="text/javascript">
		$(document).on('input', '#search', function(){
			function searchByDealerName(searchTxt){
			    $.ajax({
			        url: "../ajaxcall_rod_report/pathor_report_dealer_search.php",
			        type: "post",
		...
		function dealerWiseSearch(searchDealer){
			$.ajax({
		        url: "../ajaxcall_rod_report/pathor_report_dealer_name_wise_search.php",
		        type: "post",
		        data: {
		...
		$(document).on('change', '#searchDealer', function(){
			...
			if(searchDealer == 'alldealers'){
				window.location = '../vaucher/pathor_report_dealer.php';
			} else {

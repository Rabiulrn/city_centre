<?php 
  ...
  $sql = "SELECT header_id FROM balu_table_headers ORDER BY id DESC LIMIT 1";
  $customersId = $db->select($sql);
  if($customersId->num_rows > 0){
      $row = $customersId->fetch_assoc();
      $largestId = $row['header_id'];
  } else {
        $largestId = 'header-100000';
  }
  $matches = preg_replace('/\D/', '', $largestId);
  $newNumber = $matches + 1;
  $newId = 'header-' . $newNumber;
  ...
  if(isset($_POST['submit'])){
      ...
      if($submitBtn_value == 'Save'){
          $header_id     = $newId;
          $sql="INSERT INTO balu_table_headers (header_id, header_name) VALUES ('$header_id','$header_name')";
          ...
          if ($db->select($sql) === TRUE) {
              ...
              $sucMsg = "New Header Saved Successfully";
          } else {
              echo "Error: " . $sql . "<br>" . $db->error;
          }
      } else {
          $header_id = $_POST['header_id'];
          $sql="UPDATE balu_table_headers SET header_name = '$header_name' WHERE header_id = '$header_id'";
  ...
  if(isset($_GET['remove_id'])){
      ...
      $sql = "DELETE FROM balu_table_headers WHERE id = '$delete_dealer'";
      if ($db->select($sql) === TRUE) {
                ...
                ?>
                    <div class="project_heading">      
                        <h2 class="headingOfAllProject">
                            <?php echo $rows['heading']; ?> <span class="protidinHisab">হেডার এন্ট্রি</span>
            ...
            <div class="rodDelEnCon">        
                ...
                <form action="" method="post" onsubmit="return validation()">
                    <table class="dealersTableCon">
                        ...
                        <tr>
                          <td>
                            <input type="text" class="form-control" id="header_id" value="<?php echo $newId; ?>" disabled>
                            <input type="hidden" name= "header_id" class="form-control" id="header_id_hidden" value="<?php echo $newId; ?>">
                          </td>
                          <td><input type="text" name = "header_name" class="form-control" id="header_name" placeholder="Enter Header Name..."></td>

...
$_SESSION['pageName'] = 'brick_kroy_hisab';
// $sucMsgPopup = '';
?>
...
<body>
            ...
            <div class="dealerIdSelect">
                <table>
                    <tr>
                        ...
                        <td><?php
                            $sql = "SELECT DISTINCT dealer_name, dealer_id FROM brick_dealer WHERE dealer_name != '' AND  project_name_id = '$project_name_id'";
                        ...
                        <table style="width: 100%;">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php
                                    $sql = "SELECT dealer_id FROM brick_dealer WHERE project_name_id ='$project_name_id'";
                                    $all_custmr_id = $db->select($sql);
    ...
    <script type="text/javascript">
        function dealerWiseSummaryDetailsSearchAndEntry(dlrId, restext = false) {
            $.ajax({
                url: '../ajaxcall/brick_dealer_wise_summary_details_search_and_entry.php',
                type: 'post',
                data: {
                    dealerId: dlrId,
                },
            ...
            function ConfirmDialog(message, data_delete_id) {
                $('<div></div>').appendTo('body')
                    ...
                    .dialog({
                        ...
                        buttons: {
                            Yes: function() {
                                var urltxt = '../ajaxcall/brick_del_entry_ajax.php';
                                $.ajax({
                                    url: urltxt,
                                    type: 'post',
                                    dataType: 'html',
    ...
    <script type="text/javascript">
        function valid(submit_type) {
            ...
            if (submit_type == 'insert') {
                ...
                var formElement = $('#insertPopupForm')[0];
                var formData = new FormData(formElement);
                var urltxt = '../ajaxcall/brick_update_entry_ajax.php';
    ...
    <script type="text/javascript">
        ...
        function edit_rod_popup(element, rowid) {
            ...
            var address = $(element).closest('tr').find('td:eq(7)').text();
            var sl = $(element).closest('tr').find('td:eq(8)').text();
            var voucher_no = $(element).closest('tr').find('td:eq(9)').text();
            var motor_sl = $(element).closest('tr').find('td:eq(10)').text();
            // var address = $(element).closest('tr').find('td:eq(10)').text();
            var motor_number = $(element).closest('tr').find('td:eq(11)').text();
            ...
            var particulars = $(element).closest('tr').find('td:eq(16)').text();

            var count = $(element).closest('tr').find('td:eq(17)').text();
            var paras = $(element).closest('tr').find('td:eq(18)').text();
            var discount = $(element).closest('tr').find('td:eq(19)').text();
            var credit = $(element).closest('tr').find('td:eq(20)').text();
            // alert(buyr_id);
    ...
    <script type="text/javascript">
        ...
        $(document).on('input change paste keyup', '.value-calc', function() {
            ...
            var name = credit ? parseFloat(credit) : 0;
            var balance = name - debit;
            // alert(balance);
            ...
            if (discount != '') {
                ...
                var dis = discount ? parseFloat(discount) : 0;
                var dis_credit = name + dis;
                $('#credit').val(dis_credit.toFixed(3));
                if (dis > credit) {
                    $('#dis').focus(function() {
                        $('#dis').val("");
                    });
            ...
            var g_khalas = $("#car_rent_redeem").val();
            var credit = $("#credit").val();
            var name = credit ? parseFloat(credit) : 0;
            var name2 = g_khalas ? parseFloat(g_khalas) : 0;
            var et_paras = name + name2;
            // console.log(et_paras);
            // console.log(typeof(et_paras));
            // alert(balance);
            $('#et_para').val(et_paras.toFixed(3));
            // // if (motor_vara == '') {
            // //     $('#motor_vara').val()=null;
            // // } else if (unload == '') {
            // //     $('#unload').val()=null;

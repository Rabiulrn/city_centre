<?php 
    ...
    $_SESSION['pageName'] = 'rod_buyer_entry';   

    $sucMsg = '';
    $submitBtn_value ='';
    $sql = "SELECT buyer_id FROM buyers ORDER BY id DESC LIMIT 1";
    ...
    if(isset($_POST['submit'])){
        ...
        if($submitBtn_value === 'Save'){
            ...
            $sql="INSERT INTO buyers (buyer_id, buyer_name, address, mobile, buyer_type, project_name_id) VALUES ('$buyer_id','$buyer_name', '$address', '$mobile', '$buyer_type', '$project_name_id')";
            ...
            // var_dump($buyer_id);
            $sql="UPDATE buyers SET buyer_name = '$buyer_name', address = '$address', mobile = '$mobile' , buyer_type = '$buyer_type' WHERE buyer_id = '$buyer_id'";
    ...
    if(isset($_GET['remove_id'])){
        ...
        $sql = "DELETE FROM buyers WHERE id = '$delete_buyer'";
        if ($db->select($sql) === TRUE) {
                ...
                $('<div></div>').appendTo('body')
                                ...
                                .dialog({
                                    ...
                                    buttons: {
                                        Yes: function () {
                                            $(this).dialog("close");
                                            $.get("buyer_entry.php?remove_id="+remove_id, function(data, status){
                                              console.log(status);
                                              if(status == 'success'){
                                                window.location.href = 'buyer_entry.php';
                                              }
                                            });
                                        },
                                        No: function () {

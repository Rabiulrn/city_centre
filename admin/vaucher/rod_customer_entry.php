<?php 
	...
	$_SESSION['pageName'] = 'rod_cusotomer_entry';	
	$sucMsg = '';
	...
	if(isset($_POST['submit'])){
		...
		if($submitBtn_value == 'Save'){
			...
			$sql="INSERT INTO customers (customer_id, customer_name, address, mobile, buying_type, project_name_id) VALUES ('$customer_id', '$customer_name', '$address', '$mobile', '$buying_type', '$project_name_id')";
			...
			$sql="UPDATE customers SET customer_name = '$customer_name', address = '$address', mobile = '$mobile', buying_type = '$buying_type' WHERE customer_id = '$customer_id'";
	...
	if(isset($_GET['remove_id'])){
		...
		$sql = "DELETE FROM customers WHERE id = '$customer_delete'";
		if ($db->select($sql) === TRUE) {
...
<html>
<head>
	<title>রড কাষ্টমার এনট্রি</title>
	<meta charset="utf-8">
...
<body>
	...
	<script type="text/javascript">
		function validation(){
			...
			if(customer_name == ''){
				$('#cNameErr').html('Customer name cant be empty !');
				$('#customer_name').focus();
			} else if(customer_name.length > 100){
				$('#cNameErr').html('Customer name cant greater than 100 characters !');
				$('#customer_name').focus();
			} else if($.isNumeric(customer_name)){
				$('#cNameErr').html('Customer name cant a number !');
				$('#customer_name').focus();
			} else{
				$('#cNameErr').html('');
			...
			if(mobile == ''){
				$('#mobileErr').html('Mobile no cant be empty !');
				$('#mobile').focus();
			} else if(mobile.length > 11){
				$('#mobileErr').html('Mobile cant greater than 11 characters !');
				$('#mobile').focus();
			...
			if(buying_type == ''){
				$('#buyingTypeErr').html('Buying Type cant be empty !');
				$('#buying_type').focus();
			} else if(buying_type.length > 100){
				$('#buyingTypeErr').html('Buying Type cant greater than 100 characters !');
				$('#buying_type').focus();
			} else if($.isNumeric(buying_type)){
				$('#buyingTypeErr').html('Buying Type cant be a number !');
				$('#buying_type').focus();
			} else{
				$('#buyingTypeErr').html('');
	            ...
	            $('<div></div>').appendTo('body')
	                            ...
	                            .dialog({
	                                ...
	                                buttons: {
	                                    Yes: function () {
	                                        $(this).dialog("close");
	                                        $.get("rod_customer_entry.php?remove_id="+remove_id, function(data, status){
	                                          console.log(status);
	                                          if(status == 'success'){
	                                            window.location.href = 'rod_customer_entry.php';
	                                          }
	                                        });
	                                    },
	                                    No: function () {

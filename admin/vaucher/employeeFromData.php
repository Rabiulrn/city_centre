...
$employee_start_date = isset($_POST["employee_start_date"]) ? $_POST["employee_start_date"] : [];
$employee_end_date   = isset($_POST["employee_end_date"]) ? $_POST["employee_end_date"] : [];
$totalDay            = isset($_POST["totalDay"]) ? $_POST["totalDay"] : [];
$total_naster_bill   = isset($_POST["naster_bill"]) ? $_POST["naster_bill"] : [];
$total_khaber_bill   = isset($_POST["khaber_bill"]) ? $_POST["khaber_bill"] : [];
$total_diner_beton   = isset($_POST["diner_beton"]) ? $_POST["diner_beton"] : [];
$bonus               = isset($_POST["bonus"]) ? $_POST["bonus"] : [];
$nea_joma            = isset($_POST["nea_joma"]) ? $_POST["nea_joma"] : [];

// Ensure it’s an array before counting
$number = is_array($total_naster_bill) ? count($total_naster_bill) : 0;
if ($number > 0) {
    for ($i = 0; $i < $number; $i++) {

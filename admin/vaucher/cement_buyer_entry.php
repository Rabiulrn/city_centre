<?php 
    ...
    $_SESSION['pageName'] = 'cement_buyer_entry';   

    $sucMsg = '';
    $submitBtn_value ='';
    $sql = "SELECT buyer_id FROM cement_buyers ORDER BY id DESC LIMIT 1";
    ...
    if(isset($_POST['submit'])){
        ...
        if($submitBtn_value === 'Save'){
            ...
            $sql="INSERT INTO cement_buyers (buyer_id, buyer_name, address, mobile, buyer_type, project_name_id) VALUES ('$buyer_id','$buyer_name', '$address', '$mobile', '$buyer_type', '$project_name_id')";
            ...
            // var_dump($buyer_id);
            $sql="UPDATE cement_buyers SET buyer_name = '$buyer_name', address = '$address', mobile = '$mobile' , buyer_type = '$buyer_type' WHERE buyer_id = '$buyer_id'";
    ...
    if(isset($_GET['remove_id'])){
        ...
        $sql = "DELETE FROM cement_buyers WHERE id = '$delete_buyer'";
        if ($db->select($sql) === TRUE) {
...
<html>
<head>
  <title> সিমেন্ট বায়ার এন্ট্রি</title>
  <meta charset="utf-8">
    ...
    <div class="bar_con">
        ...
        <div class="showDealerCon">
          <table >
            ...
            <?php
              $sql = "SELECT * FROM cement_buyers  WHERE project_name_id = '$project_name_id'";
              $show = $db->select($sql);
              if ($show) {
                  while ($rows = $show->fetch_assoc()){
                      echo "<tr>";
                ...
                $('<div></div>').appendTo('body')
                                ...
                                .dialog({
                                    ...
                                    buttons: {
                                        Yes: function () {
                                            $(this).dialog("close");
                                            $.get("cement_buyer_entry.php?remove_id="+remove_id, function(data, status){
                                              console.log(status);
                                              if(status == 'success'){
                                                window.location.href = 'cement_buyer_entry.php';
                                              }
                                            });
                                        },
                                        No: function () {

...
if(isset($_POST['update'])) {
    $postDateArr  = explode('/', $_POST['dates']);
    $dates        = $postDateArr['2'].'-'.$postDateArr['1'].'-'.$postDateArr['0'];

    $credit_name = $_POST['credit_name'];
    $credit_amount = $_POST['credit_amount'];
    $query = "UPDATE vaucher_credit SET credit_name='$credit_name', credit_amount='$credit_amount', credit_date = '$dates' WHERE id = $edit_id AND project_name_id = '$project_name_id'";
...
<html lang="en">
<head>
  <title>Update Vaucher Credit</title>
  <meta charset="utf-8">
            ...
            <table class="table table-bordered table-condensed" id="dynamic_field">
                <thead>
                  <tr>
                    ...
                    <th class="text-center" width="200px">জমাঃ</th>
                  </tr>
                </thead>
                <?php  
                    
                    $query = "SELECT * FROM vaucher_credit WHERE id = '$edit_id' AND project_name_id = '$project_name_id'";
                    $show = $db->select($query);
                ...
                ?>
                        <tbody>
                          <tr>
                            <td><input type="text", name="dates" class="form-control text-center" value="<?php if($data['credit_date'] !== '0000-00-00'){echo date("d/m/Y", strtotime($data['credit_date']));} ?>" id="dates"/></td>
                            <td><input type="text", name="credit_name" class="form-control" size="100" value="<?php echo $data['credit_name']; ?>" id="mName"/></td>
                            <td><input type="text", name="credit_amount" class="form-control" size="100" value="<?php echo $data['credit_amount']; ?>" id="mAmount"/></td>                            
                          </tr>
                        </tbody>
                <?php
                    }

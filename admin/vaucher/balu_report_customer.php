<?php 
	...
	$db = new Database();
	$_SESSION['pageName'] = 'balu_report_customer';
...
?> 
	...
	<div class="bar_con">
		...
		<div class="left_side_bar menu">
			<div id="left_all_menu_con">
				...
				<!-- <a href="../vaucher/rod_report_others_category.php">রড ও অন্যান্ন ক্যাটাগরি</a> -->
				 ...
				 <a href="../vaucher/balu_report_customer.php" class="active">কাস্টমার অনুযায়ী </a> 
		...
		<div class="main_bar">
    		...
    		<table class="tableshow" id="tableshow" style="border-collapse: collapse; border: 1px solid #777 !important;">
					...
					<th style='border: 1px solid #777 !important;'>কাস্টমার নাম</th>
    			...
    			<?php
    				...
    				if($result && $row_number > 0){
    					...
    					while($row = $result->fetch_assoc()){
    						...
    						// $sql2 = "SELECT SUM(kg) as kg FROM details_sell_balu WHERE particulars LIKE '%500W%' AND dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
					        ...
					        //     $rod500w = 0;
					        // }
					        // $sql2 = "SELECT SUM(kg) as kg FROM details_sell_balu WHERE particulars LIKE '%400W%' AND dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
					        // $result2 = $db->select($sql2);
					        // if($result2->num_rows > 0){
					        //     while($row2 = $result2->fetch_assoc()){
					        //         $rod400w = $row2['kg'];
					        ...
					        // 	$total_motor = 0;
						    //     $sql2 = "SELECT SUM(motor) as motor FROM details_sell_balu WHERE dealer_id = '$dealer_id' AND project_name_id = '$project_name_id'";
						    //     $result2 = $db->select($sql2);
						    //     if($result2->num_rows > 0){
						    //         while($row2 = $result2->fetch_assoc()){
					        ...
					        //Start Gari vara
								...
								$total_motor_vara = 0;
						        $sql3 = "SELECT SUM(motor_vara) as motor_vara FROM details_sell_balu WHERE project_name_id = '$project_name_id'";
						    ...
						    //Start khalas/Unload
								...
								$total_unload = 0;
								$sql3 = "SELECT SUM(unload) as unload FROM details_sell_balu WHERE  project_name_id = '$project_name_id'";
								$result3 = $db->select($sql3);
								if($result3->num_rows > 0){
									while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_credit/mot_mul
								...
								$total1_credit = 0;
						        $sql3 = "SELECT SUM(credit) as credit FROM details_sell_balu WHERE project_name_id = '$project_name_id'";
						        $result3 = $db->select($sql3);
						        if($result3->num_rows > 0){
						            while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_debit/joma
						        ...
						        if($result2->num_rows > 0){
								  ...
								  $total1_debit = 0;
						          $sql3 = "SELECT SUM(debit) as debit FROM details_sell_balu WHERE project_name_id = '$project_name_id'";
						          $result3 = $db->select($sql3);
						          if($result3->num_rows > 0){
						              while($row3 = $result3->fetch_assoc()){
						    ...
						    // Start total total_Balance/mot_jer
								...
								$total1_balance = 0;
						        $sql3 = "SELECT SUM(balance) as balance FROM details_sell_balu WHERE  project_name_id = '$project_name_id'";
						        $result3 = $db->select($sql3);
						        if($result3->num_rows > 0){
						            while($row3 = $result3->fetch_assoc()){
	...
	<script type="text/javascript">	
		...
		function fromTodateSearch(fromdate, todate){
			$.ajax({
		        url: "../ajaxcall_rod_report/balu_report_customer_fromdate_todate_search.php",
		        type: "post",
		        data: {
		        	fromdate 	: fromdate, 
		        	todate 		: todate,
		...
		function yearMonthSearch(monthvalue, yearvalue){
			$.ajax({
		        url: "../ajaxcall_rod_report/balu_report_customer_year_month_search.php",
		        type: "post",
		        data: {
	...
	<script type="text/javascript">
		$(document).on('input', '#search', function(){
			function searchByCustomerName(searchTxt){
			    $.ajax({
			        url: "../ajaxcall_rod_report/balu_report_customer_search.php",
			        type: "post",
		...
		function customerWiseSearch(searchCustomer){
			$.ajax({
		        url: "../ajaxcall_rod_report/balu_report_customer_name_wise_search.php",
		        type: "post",
		        data: {

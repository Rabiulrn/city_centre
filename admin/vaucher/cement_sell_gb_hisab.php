...
$sql = "SELECT bg_id FROM  cement_sell_gb_bank_history ORDER BY id DESC LIMIT 1";
$bgsId = $db->select($sql);
if($bgsId->num_rows > 0){
...
if (isset($_POST['submit'])) {
	...
	if ($submitBtn_value == 'Save') {
		...
		$sql = "INSERT INTO cement_sell_gb_bank_history (bg_id,customer_id,bank_name, branch, value, time, document, project_name_id) VALUES ('$bg_id','$customer_id','$bank_name', '$branch', '$amount', now(), '$amount', '$project_name_id')";
		...
		$bg_id = $_POST['bg_id'];
		echo $bg_id;
		$sql = "UPDATE cement_sell_gb_bank_history SET bank_name = '$bank_name', branch = '$branch', value = '$amount' WHERE bg_id = '$bg_id'";
...
if (isset($_GET['remove_id'])) {
	...
	$sql = "DELETE FROM cement_sell_gb_bank_history WHERE id = '$customer_delete'";
	if ($db->select($sql) === TRUE) {
		...
		<div class="main_bar">
					...
					<table class="customerEntryTable">
						...
						<td style="display: none;">
							...
							<td>
								...
								echo '<select name="customer_id" id="customer_id" class="form-control" style="width: 140px; required">';
			...
			<div class="showCustomerCon">
				<table>
					...
					<?php
					$sql = "SELECT * FROM cement_sell_gb_bank_history";
					$show = $db->select($sql);
					if ($show) {
						while ($rows = $show->fetch_assoc()) {
							echo "<tr>";
				...
				$('<div></div>').appendTo('body')
					...
					.dialog({
						...
						buttons: {
							Yes: function() {
								$(this).dialog("close");
								$.get("cement_sell_gb_hisab.php?remove_id=" + remove_id, function(data, status) {
									console.log(status);
									if (status == 'success') {
										window.location.href = 'cement_sell_gb_hisab.php';
									}
								});
							},
							No: function() {

<?php 
  ...
  $_SESSION['pageName'] = 'brick_hisab_entry';  
  $sucMsg = '';

  $sql = "SELECT category_id FROM brick_category ORDER BY id DESC LIMIT 1";
  ...
  if(isset($_POST['submit'])){
      ...
      if($submitBtn_value == 'Save'){
          $category_id     = $newId;
          $sql="INSERT INTO brick_category (category_id, category_name,project_name_id) VALUES ('$category_id','$category_name','$project_name_id')";
          ...
          $sql="UPDATE brick_category SET category_name = '$category_name' WHERE category_id = '$category_id'";
          $sql2="UPDATE details_brick SET particulars = '$category_name' WHERE particulars_id = '$category_id'";
          $db->update($sql2);
  ...
  if(isset($_GET['remove_id'])){
      ...
      $sql = "DELETE FROM brick_category WHERE id = '$delete_dealer'";
      if ($db->select($sql) === TRUE) {
...
<html>
<head>
    <title> ইট ক্যাটাগরি এনট্রি</title>
    <meta charset="utf-8">
    ...
    <div class="bar_con">
        ...
        <div class="showDealerCon">
          <table >
            ...
            <?php
              $sql = "SELECT * FROM brick_category WHERE project_name_id = '$project_name_id' ";
              $show = $db->select($sql);
              if ($show) {
                  while ($rows = $show->fetch_assoc()){
                      echo "<tr>";
                ...
                $('<div></div>').appendTo('body')
                                ...
                                .dialog({
                                    ...
                                    buttons: {
                                        Yes: function () {
                                            $(this).dialog("close");
                                            $.get("brick_hisab_entry.php?remove_id="+remove_id, function(data, status){
                                              console.log(status);
                                              if(status == 'success'){
                                                window.location.href = 'brick_hisab_entry.php';
                                              }
                                            });
                                        },
                                        No: function () {

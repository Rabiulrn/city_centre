...
$_SESSION['pageName'] = 'piling_kroy_hisab';
// $sucMsgPopup = '';
?>
...
<head>
    <title>পাইলিং বিল জমা হিসাব</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="bar_con">
        <div class="left_side_bar">
            <?php require '../others_page/left_menu_bar_piling_hisab.php'; ?>
        </div>
            ...
            ?>
                    <div class="project_heading">
                        <h2 class="headingOfAllProject" id="city_center_id">
                            <?php echo $rows['heading']; ?> <span class="protidinHisab">পাইলিং বিল জমা হিসাব</span>
            ...
            <div class="dealerIdSelect">
                <table>
                    <tr>
                        ...
                        <td><?php
                            $sql = "SELECT DISTINCT dealer_name, dealer_id FROM piling_dealer WHERE dealer_name != '' AND  project_name_id = '$project_name_id'";
            ...
            <div id="control_all">
                ...
                <div class="items_all_con" style="background-color: gray; color: white; border: 2px solid black;">
                    ...
                    <table style="width: 100%;">
                            ...
                            <!-- <input type="hidden" name="piling_details_id" id="piling_details_id"> -->
                            <tr>
                                <td>SL (ক্রমিক)</td>
                                <td>
                            ...
                            <tr>
                                <td>Cape No (কেপ নাম)</td>
                                <td>
                                    <input type="text" name="cape_no" class="form-control" id="cape_no_popup" placeholder="Enter Driver Name...">
                                </td>
                            </tr>
                            <tr>
                                <td>Bill Cash (জমা টাকা)</td>
                                <td>
                            ...
                            <tr>
                                <td>Piling Count (পরিমান‌)</td>
                                <td>
                                    <input type="text" name="count" class="form-control value-calc-popup" id="count_popup" placeholder="Enter count...">
                                </td>
                            </tr>
                            <tr>
                                <td>Feet (ফিট)</td>
                            ...
                            <tr>
                                <td>Piling Bill (পাইলিং বিলঃ)</td>
                                <td>
                                    <input type="text" onkeypress="return isNumber(event)" name="piling_bill" class="form-control value-calc-popup" id="piling_bill_popup" placeholder="Enter Motor Vara...">
                                </td>
                            </tr>
                            <!-- <tr>
                                <td>id</td>
                            ...
                            <tr>
                                <td>Excess money back (অতিরিক্ত টাকা ফেরত)</td>
                                <td>
                                    <input type="text" onkeypress="return isNumber(event)" name="money_back" class="form-control value-calc-popup" id="money_back_popup" placeholder="Unload">
                                </td>
                            </tr>
                        ...
                        //         $id2 = $row['id'];
                        //        echo '<input type="hidden" name="piling_details_id" id="piling_details_id" value="' . $id2 . '">' ;
                        //     }
                        // } 
                        ?>
                        <input type="hidden" name="piling_details_id" id="piling_details_id">
    ...
    <script type="text/javascript">
        function dealerWiseSummaryDetailsSearchAndEntry(dlrId, restext = false) {
            $.ajax({
                url: '../ajaxcall/piling_dealer_wise_summary_details_search_and_entry.php',
                type: 'post',
                data: {
                    dealerId: dlrId,
                },
        ...
        function getDealerNameByDealerId(dlrIda) {
            $.ajax({
                url: '../ajaxcall/piling_get_dealer_name_by_dealer_id.php',
                type: 'post',
                data: {
                    dealerId: dlrIda,
                },
            ...
            function ConfirmDialog(message, data_delete_id) {
                $('<div></div>').appendTo('body')
                    ...
                    .dialog({
                        ...
                        buttons: {
                            Yes: function() {
                                var urltxt = '../ajaxcall/piling_del_entry_ajax.php';
                                $.ajax({
                                    url: urltxt,
                                    type: 'post',
                                    dataType: 'html',
    ...
    <script type="text/javascript">
        function valid(submit_type) {
            ...
            if (submit_type == 'insert') {
                ...
                var urltxt = '../ajaxcall/piling_details_entry_ajax.php';
                ...
                var formElement = $('#insertPopupForm')[0];
                var formData = new FormData(formElement);
                var urltxt = '../ajaxcall/piling_update_entry_ajax.php';
    ...
    <script type="text/javascript">
        function edit_rod_details(rod_id) {
            ...
            var urltxt = '../ajaxcall/piling_edit_entry_ajax.php';
            $.ajax({
                url: urltxt,
                type: 'post',
                dataType: 'html',
        ...
        function edit_rod_popup(element, rowid) {
            var dealer_id = $(element).closest('tr').find('td:eq(0)').text();
            // var dlar_id         = $(element).closest('tr').find('td:eq(1)').text();
            var sl = $(element).closest('tr').find('td:eq(2)').text();
            var date = $(element).closest('tr').find('td:eq(1)').text();
            var marfot_name = $(element).closest('tr').find('td:eq(3)').text();
            var description = $(element).closest('tr').find('td:eq(4)').text();
            var category = $(element).closest('tr').find('td:eq(5)').text();
            var cap_no = $(element).closest('tr').find('td:eq(6)').text();
            var bill_cash = $(element).closest('tr').find('td:eq(7)').text();
            var address = $(element).closest('tr').find('td:eq(10)').text();
            var piling_count = $(element).closest('tr').find('td:eq(8)').text();
            var feet = $(element).closest('tr').find('td:eq(12)').text();
            var paras = $(element).closest('tr').find('td:eq(11)').text();
            // var delivery_date = $(element).closest('tr').find('td:eq(12)').text();
            var piling_bill = $(element).closest('tr').find('td:eq(15)').text();
            var money_back = $(element).closest('tr').find('td:eq(16)').text();
            var date = $(element).closest('tr').find('td:eq(17)').text();
            var partculars = $(element).closest('tr').find('td:eq(18)').text();
            var particulars = $(element).closest('tr').find('td:eq(19)').text();
            var debit = $(element).closest('tr').find('td:eq(20)').text();
            var monthly_com = $(element).closest('tr').find('td:eq(21)').text();
            var yearly_com = $(element).closest('tr').find('td:eq(22)').text();
            var count = $(element).closest('tr').find('td:eq(24)').text();
            var free = $(element).closest('tr').find('td:eq(25)').text();
            var count2 = $(element).closest('tr').find('td:eq(26)').text();
            ...
            var paras = $(element).closest('tr').find('td:eq(27)').text();
            var discount = $(element).closest('tr').find('td:eq(28)').text();
            var credit = $(element).closest('tr').find('td:eq(29)').text();
            var balance = $(element).closest('tr').find('td:eq(30)').text();
            ...
            // $('#piling_details_id').val(rowid);


            // $('#dealer_id_popup').val(dealer_id);
            // $('#sl_popup').val(sl);
            // $('#dates_popup').val(date);
            // $('#marfot_name_popup').val(marfot_name);
            // $('#description_popup').val(description);
            // $('#category_popup').val(category);
            // $('#cape_no_popup').val(cape_no);
            // $('#bill_cash_popup').val(bill_cash);
            // $('#address_popup').val(address);
            // $('#piling_count_popup').val(piling_count);
            // $('#feet_popup').val(feet);
            // $('#paras_popup').val(paras);
            // // $('#delivery_date_popup').val(delivery_date);
            // $('#piling_bill_popup').val(piling_bill);
            // $('#money_back_popup').val(money_back);
            ...
            $('#monthly_com_popup').val(monthly_com);
            $('#yearly_com_popup').val(yearly_com);
            $('#count_popup').val(count);
            $('#free_popup').val(free);
            $('#count2_popup').val(count2);
            // $('#ton_kg_popup').val(ton_kg);
            // $('#count_popup').val(length);
    ...
    <script type="text/javascript">
        ...
        $(document).on('input change paste keyup', '.value-calc-popup', function() {
            ...
            var count_popup = $('#count_popup').val();
            var free_popup = $('#free_popup').val();
            var count_minus_free_popup = count_popup - free_popup;
            $('#count2_popup').val(count_minus_free_popup);
            var count2_popup = $('#count2_popup').val();
            var paras_popup = $('#paras_popup').val();

            var weight_popup = count_popup / 20;
            $('#weight_popup').val(weight_popup.toFixed(2));
            ...
            if (count_popup == '') {
                ...
                var credit_popup = count2_popup * paras_popup;
                //  alert(credit);
            ...
            var monthly_com_popup = $("#monthly_com_popup").val();
            var yearly_com_popup = $("#yearly_com_popup").val();
            var monthly_com_popup = $("#monthly_com_popup").val();
            var yearly_com_popup = $("#yearly_com_popup").val();
            // if (debit == '') {
            //     // $('#balance').val('0');
            ...
            //     $('#balance').val('0');
            // } else {
            var name_popup = monthly_com_popup ? parseFloat(monthly_com_popup) : 0;
            var name2_popup = yearly_com_popup ? parseFloat(yearly_com_popup) : 0;
            var commision_popup = name_popup + name2_popup;
            // var commision = parseFloat(monthly_com) + parseFloat(yearly_com) ;
            var balance_popup = (debit_popup - credit_popup) + commision_popup;
            // alert(balance);
            $('#balance_popup').val(balance_popup.toFixed(3));


            var cedi_popup = $('#credit_popup').val();
            var cedi2_popup = parseFloat(cedi_popup);
            var credit_car_rent_popup = parseInt(car_rent_redeem_popup) + parseFloat(cedi_popup);
            $('#total_credit_popup').val(credit_car_rent_popup);




            var fee = $("#free_popup").val();
    ...
    <script type="text/javascript">
        function getDataByDates(datestr, dealerId) {
            $.ajax({
                url: '../ajaxcall/piling_search_date_entry.php',
                type: 'post',
                data: {
                    optionDate: datestr,
                    dealerId: dealerId,
        ...
        function getSummaryByDates(datestr, dealerId) {
            $.ajax({
                url: '../ajaxcall/piling_search_date_wise_summary_entry.php',
                type: 'post',
                data: {
                    optionDate: datestr,
                    dealerId: dealerId,
    ...
    <script type="text/javascript">
        ...
        function gbbank_update(id, gbvalue, dealerId) {
            $.ajax({
                url: '../ajaxcall_save_update/piling_gb_bank_update.php',
                type: 'post',
                data: {
                    details_id: id,
                    gbvalue: gbvalue,
        ...
        $(document).on('keypress change', '#gb_bank_ganti', '#delear_id', function(e) {
            if (e.which == 13) {
                ...
                // alert(id);
                gbbank_update(id, gbvalue, dealerId);
                $('#gb_bank_ganti').hide();
            }
        });
    </script>
    ...
    <script type="text/javascript">
        ...
        $('#challan_date_popup').bind('keydown', function(e) {
            if (e.which == 13)
                e.stopImmediatePropagation();
        }).datepicker({
            onSelect: function(date) {
        ...
        $('#so_date_popup').bind('keydown', function(e) {
            if (e.which == 13)
                e.stopImmediatePropagation();
        }).datepicker({
            onSelect: function(date) {

<?php 
	...
	$_SESSION['pageName'] = 'cement_customer_entry';	
	$sucMsg = '';


  	$sql = "SELECT customer_id FROM customers_cement ORDER BY id DESC LIMIT 1";
	...
	if(isset($_POST['submit'])){
		...
		if($submitBtn_value == 'Save'){
			...
			$sql="INSERT INTO customers_cement (customer_id, customer_name, address, mobile, buying_type, project_name_id) VALUES ('$customer_id', '$customer_name', '$address', '$mobile', '$buying_type', '$project_name_id')";
			...
			$sql="UPDATE customers_cement SET customer_name = '$customer_name', address = '$address', mobile = '$mobile', buying_type = '$buying_type' WHERE customer_id = '$customer_id'";
	...
	if(isset($_GET['remove_id'])){
		...
		$sql = "DELETE FROM customers_cement WHERE id = '$customer_delete'";
		if ($db->select($sql) === TRUE) {
...
<html>
<head>
	<title>সিমেন্ট কাষ্টমার এনট্রি</title>
	<meta charset="utf-8">
	  	...
	  	<div class="main_bar">
			...
			<div class="showCustomerCon">
				<table >
					...
					<?php
						$sql = "SELECT * FROM customers_cement";
						$show = $db->select($sql);
						if ($show) 
						{
							while ($rows = $show->fetch_assoc()){
	            ...
	            $('<div></div>').appendTo('body')
	                            ...
	                            .dialog({
	                                ...
	                                buttons: {
	                                    Yes: function () {
	                                        $(this).dialog("close");
	                                        $.get("cement_customer_entry.php?remove_id="+remove_id, function(data, status){
	                                          console.log(status);
	                                          if(status == 'success'){
	                                            window.location.href = 'cement_customer_entry.php';
	                                          }
	                                        });
	                                    },
	                                    No: function () {

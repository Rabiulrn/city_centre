<?php 
  ...
  $_SESSION['pageName'] = 'cement_dealer_entry';  
  $sucMsg = '';

  $sql = "SELECT dealer_id FROM cement_dealer ORDER BY id DESC LIMIT 1";
  ...
  if(isset($_POST['submit'])){
      ...
      if($submitBtn_value == 'Save'){
          $dealer_id     = $newId;
          $sql="INSERT INTO cement_dealer (dealer_id, dealer_name, address, contact_person_name, mobile, project_name_id) VALUES ('$dealer_id','$dealer_name', '$address', '$contact_person_name', '$mobile',  '$project_name_id')";
          ...
          $sql="UPDATE cement_dealer SET dealer_name = '$dealer_name', address = '$address', contact_person_name = '$contact_person_name', mobile = '$mobile' WHERE dealer_id = '$dealer_id'";
  ...
  if(isset($_GET['remove_id'])){
      ...
      $sql = "DELETE FROM cement_dealer WHERE id = '$delete_dealer'";
      if ($db->select($sql) === TRUE) {
...
<html>
<head>
    <title>সিমেন্ট ডিলার এনট্রি</title>
    <meta charset="utf-8">
                ...
                $('<div></div>').appendTo('body')
                                ...
                                .dialog({
                                    ...
                                    buttons: {
                                        Yes: function () {
                                            $(this).dialog("close");
                                            $.get("cement_dealer_entry.php?remove_id="+remove_id, function(data, status){
                                              console.log(status);
                                              if(status == 'success'){
                                                window.location.href = 'cement_dealer_entry.php';
                                              }
                                            });
                                        },
                                        No: function () {

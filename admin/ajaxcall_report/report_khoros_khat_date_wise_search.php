	...
	if($searchdate == 'alldates'){
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $mot_bill = 0;
                $mot_joma = 0;
                $sql = "SELECT * FROM debit_group_data WHERE group_id = '$debit_group_id' AND project_name_id = '$project_name_id'";
                $rslt = $db->select($sql);
                if($rslt){
            ...
            echo $table_start;
                echo '<tr><td>Data is not found !</tr></td>';
            echo "</table>";
        }
	} else {
        $query = "SELECT * FROM debit_group WHERE project_name_id = '$project_name_id'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $mot_bill = 0;
                $mot_joma = 0;
                $sql = "SELECT * FROM debit_group_data WHERE group_id = '$debit_group_id' AND entry_date = '$searchdate' AND project_name_id = '$project_name_id'";
                $rslt = $db->select($sql);

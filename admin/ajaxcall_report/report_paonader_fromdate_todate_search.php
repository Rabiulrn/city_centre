<?php
	...
	if($fromdate == ''){
		echo '<tr><td style="border: 1px solid #ddd !important; padding:3px 10px;">Please set from date !</td></tr>';
	} else if($todate == ''){
		echo '<tr><td style="border: 1px solid #ddd !important; padding:3px 10px;">Please set to date !</td></tr>';
	} else {		
        ...
        if ($result && $main_row_number > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $sql = "SELECT * FROM entry_jara_pabe WHERE  jp_date BETWEEN '$fromdateNew' AND '$todateNew' AND jara_pabe_id ='$pabe_id' AND project_name_id = '$project_name_id' ORDER BY jp_date ASC";
                $rslt = $db->select($sql);
                // $row_count =0;
                // $found_row_print++;

    ...
    if($monthvalue == ''){
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $sql = "SELECT * FROM debit_group_data WHERE group_id = '$debit_group_id' AND entry_date LIKE '$yearvalue%' AND project_name_id = '$project_name_id'";
                $rslt = $db->select($sql);
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $mot_joma = 0;
                $not_found_first_time_debit_group_data++;
                $sql = "SELECT * FROM debit_group_data WHERE group_id = '$debit_group_id' AND entry_date LIKE '$dateMatch%' AND project_name_id = '$project_name_id'";
                $rslt = $db->select($sql);
                if($rslt && mysqli_num_rows($rslt) > 0){                    
                    // start header print
                        echo "<tr class='tableGroupColor' style='border: 1px solid #ddd; background-color: #8e8e8e;'>";

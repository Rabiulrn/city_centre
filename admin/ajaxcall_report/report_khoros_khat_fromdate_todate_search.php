<?php
	...
	if($fromdate == ''){
		echo $table_start . "<tr><td style='text-align:center;'>Please set &ldquo;From&rdquo; date !</tr></td></table>";
	} else if($todate == ''){
		echo $table_start . "<tr><td style='text-align:center;'>Please set &ldquo;To&rdquo; date !</tr></td></table>";
	} else {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = $result->fetch_assoc()){
                ...
                $sql = "SELECT * FROM debit_group_data WHERE group_id = '$debit_group_id' AND entry_date BETWEEN '$fromdateNew' AND '$todateNew' AND project_name_id = '$project_name_id'";
                $rslt = $db->select($sql);

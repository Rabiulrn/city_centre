<?php 
  ...
  $db = new Database();
  $_SESSION['pageName'] = 'page_setting';
  $ph_id = $_SESSION['project_name_id'];
    ...
    $newImageName = $newTime ."_". "banner_image" . "." .$imageFileType;  //Generate New Image name   
    ...
    // if everything is ok, try to upload file
          ...
          if($moveResult !== true){
            echo "Image Not moved !";
              ...
              $sql = "UPDATE project_heading SET banner_img_name = '$newImageName' WHERE id = '$ph_id'";
...
<body>
    ...
    <div class="container"> 
      ...
      <form action="" method="POST" enctype="multipart/form-data" onsubmit="return validation()">
        <div class="psContainer">
        	<h3 class="text-center bg-primary" style="margin-top: 0px; padding: 5px;">Change Banner Image</h3>
        	<div class="inputCon">
        		<label>Select Banner Image: </label>
        		<input type="file" name="imageUpload" id="imgFile" onchange="loadFile(event)">       		
        	</div>
        	<img src="../img/banner/default_banner_img.jpg" class="imgView" id="imgShowId"/>
        	
        	
        	<input type="submit" name="submit" value="Upload New Banner Image" class="btn btn-block btn-primary">
    ...
    <script>
		...
		function validation(){
		  	...
		  	if(imgFile == ''){
		  		validRetrun = false;
		  		$('#errMsgId').html('Please select an image for banner !').css({'text-align':'center', 'margin-top' : '15px'});
		  	} else{
		  		validRetrun = true;
		  	}
